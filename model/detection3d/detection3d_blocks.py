...
class SparseBox3DEncoder(nn.Module):
    ...
    Encodes 3D box parameters (position, size, yaw, velocity) into embeddings.
    """
    ...
    def forward(self, box_3d: torch.Tensor):
        ...
        Args:
            box_3d: 3D box tensor [x, y, z, w, l, h, sin_yaw, cos_yaw, vx, vy, vz]

        Returns:
            Encoded box features
        """
...
class SparseBox3DRefinementModule(nn.Module):
    """Module for refining 3D box predictions.

    Refines predicted 3D boxes using instance features and anchor boxes.
    """
    ...
    def forward(
        ...
        Args:
            ...
            time_interval: Time interval for velocity computation
            return_cls: Whether to return classification scores

        Returns:
            Tuple of (refined_boxes, cls_scores, quality_scores)
        """
...
class SparseBox3DKeyPointsGenerator(nn.Module):
    """Generator for 3D box keypoints.

    Generates keypoints from 3D boxes for sampling features.
    """
    ...
    def forward(
        ...
        Args:
            anchor: Anchor boxes
            instance_feature: Instance features for learnable keypoints
            T_cur2temp_list: List of transformation matrices
            cur_timestamp: Current timestamp
            temp_timestamps: List of temporal timestamps

        Returns:
            Keypoints in current frame, or tuple of (current_keypoints, temporal_keypoints_list)
        """
    ...
    def anchor_projection(
        ...
        """Project anchors to different frames.

        Args:
            anchor: Source anchor boxes
            T_src2dst_list: List of transformation matrices
            src_timestamp: Source timestamp
            dst_timestamps: List of destination timestamps
            time_intervals: List of time intervals

        Returns:
            List of projected anchors
        """
        dst_anchors = []
    ...
    @staticmethod
    def distance(anchor):
        """Compute distance of anchors from origin.

        Args:
            anchor: Anchor boxes

        Returns:
            L2 distance in XY plane
        """
