...
class SparseBox3DDecoder(object):
    """Decoder for 3D box predictions.

    Decodes predicted 3D boxes and classification scores into final detections.
    """
    ...
    def decode_box(self, box):
        """Decode box representation.

        Converts internal box representation to standard format:
        [x, y, z, w_exp, l_exp, h_exp, yaw, vx, vy, vz]

        Args:
            box: Box tensor with log dimensions and sin/cos yaw
    ...
    def decode(
        ...
        """Decode predictions into final detections.

        Args:
            cls_scores: Classification scores
            box_preds: Box predictions
            instance_id: Optional instance IDs
            qulity: Optional quality scores
            output_idx: Index of output to decode (-1 for last)

        Returns:
            List of detection dictionaries with keys:
                - boxes_3d: Decoded 3D boxes
                - scores_3d: Detection scores
                - labels_3d: Class labels
                - instance_ids: (optional) Instance IDs
                - cls_scores: (optional) Original classification scores
        """
